...
plot_scatter_updated(conf_strengths_1, [- elem for elem in triple_re_best], 
                     "Conf. Strength vs. Best RPI Reweighting\nRank Error (Inverted), 2024 Season", 
...
# Data
corrs = [0.9157, 0.8229, 0.7654, 0.7845, 0.6885, 0.4559, 0.2057, 0.8587]
...
# Create bar colors
colors = ['green', 'skyblue', 'skyblue', 'skyblue', 'skyblue', 'skyblue', 'skyblue', 'purple']
...
plt.title("Correlation (R) Between Estimated Conf. Strength and Rank\nError (Inverted), Std. RPI and Selected Weightings, 2024 Season")
...
print(acc_labels)
accuracy_values = [0.760905, 0.769924, 0.767845, 0.765073, 0.76438, 0.763687, 0.762994, 0.762994, 0.776161]
...
labels = acc_labels
colors = ['orange', 'green'] + ['skyblue'] * 6 + ['purple']
...
plt.ylabel("Accuracy")
plt.title("Predictive Accuracy of Rankings for Bradley-Terry,\nStd. RPI and Selected Weightings, 2024 Season")
plt.ylim(0.75, 0.785)
plt.tight_layout()
plt.show()
...
teams = [
["Modified RPI Rank (0.15*WP\n+ 0.3*OWP + 0.55*OOWP)", "Team", "Conference", "Rank Diff. From RPI"],
...
table = ax.table(cellText=teams, loc='center')
...
for (row, col), cell in table.get_celld().items():
    if col == 1 or col == 0:
        ...
        cell.set_width(0.2)  # Wider first column
    else:
        cell.set_width(0.25)  # Narrower others
    if row == 0:
        cell.set_height(0.1)
    else:
