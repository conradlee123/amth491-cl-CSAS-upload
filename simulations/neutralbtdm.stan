...
data {
  ...
  //^ adjust to allow -1
  array[N] int<lower=0, upper=1> neutral; // 1 = played on neutral site
}
parameters {
...
model {
  ...
  for (n in 1:N) {
    ...
    real param_home = exp(alpha[player1[n]] + (rho * (1 - neutral[n]))); //numerator for home win
    //real param_home = exp(alpha[player1[n]] + rho);
