...
for sim in range(num_simulations):
    ...
    ###Synthetic generation of matchups####
    #"""
    player0, player1 = synthetic_matchups.generate_random_pairings(teams_dict, 8)
    player_conf0, player_conf1 = synthetic_matchups.generate_conference_pairings(teams_dict, False)
    p0_names = player0 + player_conf0
    ...
    #"""
    ### END synthetic generation of results ###
    ...
    rpi_triple_rank = [0] * len(rpi_triples)

    trip_num = -1
    for tripl in rpi_triples:
        trip_num = trip_num + 1
        ### Calculate RPI ###
        rpi_full_values = calculate_better_rpi(sim_data, tripl)
        ...
        rpi_r_s[sim] = np.flip(np.argsort(rpi_values_np))
        rpi_triple_rank[trip_num] = rpi_r_s[sim] + 1




    bt_confusion[sim] = [confusion_matrix(elem, sim_data) for elem in rpi_triple_rank]
        #"""
        ### End Calculate RPI ###
...
for tripl in rpi_triples:
    trip_num = trip_num + 1
    rpi_trip_rank_mean.append(np.mean(np_bt_confusion[:, trip_num, 0]))
    rpi_trip_rank_var.append(np.var(np_bt_confusion[:, trip_num, 0]))
...
rpi_index = rpi_triples.index((0.4, 0.4, 0.2))
print("442 Triple:")
print(rpi_triples[rpi_index])
print("Average Accuracy")
print(rpi_trip_rank_mean[rpi_index])
...
#print(rpi_triples[:30])

"""
for tripl in range(len(rpi_triples)):
    print("Triple:")
    print(rpi_triples[tripl])
    print("Average Accuracy")
    #TODO!!!!!!!!!!
    print(rpi_trip_rank_mean[tripl])
"""
