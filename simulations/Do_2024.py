...
for sim in range(num_simulations):
    ...
    rpi_values_n = np.array(rpi_values)
    rpi_r_s[sim] = np.flip(np.argsort(rpi_values_n))
    rpi_r_s[sim] = rpi_r_s[sim] + 1

    ### Adjusted RPI (if needed) ###
    #"""
    adjusted_rpi_values = adjusted_rpi(sim_data, rpi_values, rpi_r_s[sim], [])
    rpi_values_ns = np.array(adjusted_rpi_values)
    rpi_r_s[sim] = np.flip(np.argsort(rpi_values_ns))
    rpi_r_s[sim] = rpi_r_s[sim] + 1
    #"""
...
#print(conf_names)
print("Predicted Team Rankings (best to worst)")
...
np_triples_er_ = [0] * len_triples
np_triples_confusion = [0] * len_triples
for trip in range(len_triples):
    np_triples_er_[trip] = np.array(triples_er_[trip])
    np_triples_confusion[trip] = np.array(triples_confusion[trip])
    triples_rank_error[trip] = np.mean(np_triples_er_[trip], axis=0)
...
print("Variance of above BT spearman's coefficient")
print(round(np.var(bt_spearmans), 4))
print("Average BT Accuracy")
print(round(np.mean(np_bt_confusion[:, 0]), 6))
print("Variance of BT Accuracy")
print(round(np.var(np_bt_confusion[:, 0]), 6))
...
print(round(np.mean(np_rpi_confusion[:, 0]), 6))
print("Variance of RPI Accuracy")
print(round(np.var(np_rpi_confusion[:, 0]), 6))
...
trip_num = -1
#print(np_bt_confusion)
rpi_trip_rank_mean = []
rpi_trip_rank_var = []
for tripl in rpi_triples:
    trip_num = trip_num + 1
    rpi_trip_rank_mean.append(np.mean(np_triples_confusion[trip_num][:, 0]))
    rpi_trip_rank_var.append(np.var(np_triples_confusion[trip_num][:, 0]))



top_5_indices = np.argsort(rpi_trip_rank_mean)[-5:][::-1] 
worst_5_indices = np.argsort(rpi_trip_rank_mean)[:5]
...
#for index in top_5_indices:
    ...
    #print("Triple:")
    #print(rpi_triples[index])
    #print("Average Accuracy")
    #print(rpi_trip_rank_mean[index])
    #print("Average Variance")
    #print(rpi_trip_rank_var[index])
#print("")

"""
rpi_index = rpi_triples.index((0.25, 0.5, 0.25))
print("RPI Triple:")
print(rpi_triples[rpi_index])
print("Average Accuracy")
print(rpi_trip_rank_mean[rpi_index])
print("Average Variance")
print(rpi_trip_rank_var[rpi_index])
"""
...
for trip in top_5_indices:
    trip_name = str(modified_rpi_triples[trip])
    print("Triple: ")
    print(trip_name)
    print("Rank error for " + trip_name + " by conference (averaged)")
    #rpi_rank_error = np.mean(np_triples_er_[trip], axis=0)
    print([round(num, 4) for num in triples_rank_error[trip]])
    print("Correlation coefficient (R) between conference strength and " + trip_name + " rank error")
    r_trip, p_trip = pearsonr(list(conf_strengths.values()), triples_rank_error[trip])
    print(round(r_trip, 4))
    print("P-value for correlation between conference strength and " + trip_name + "rank error")
    print(round(p_trip, 4))


    print("Average spearman's coefficient for " + trip_name + " rankings, compared to ground truth")
    print(round(sum(triples_spearmans[trip]) / num_simulations, 4))
    print("Variance of above " + trip_name + " spearman's coefficient")
    print(round(np.var(triples_spearmans[trip]), 4))
    print("Average " + trip_name + " Accuracy")
    print(round(np.mean(np_triples_confusion[trip][:, 0]), 6))
    print("Variance of " + trip_name + " Accuracy")
    print(round(np.var(np_triples_confusion[trip][:, 0]), 6))

    print("Correlation coefficient (R) between conference strength and " + trip_name + " rank error - BTDM rank error")
    r_trip, p_trip = pearsonr(list(conf_strengths.values()), [triples_rank_error[trip][elem] - btdm_rank_error[elem] for elem in range(len(btdm_rank_error))])
    print(round(r_trip, 4))
    print("P-value for correlation between conference strength and " + trip_name + " rank error - BTDM rank error")
    print(round(p_trip, 4))
    print("")
...
top_5_indices = [rpi_triples.index(elem) for elem in test_rpi_triples]
...
#rpi_final_list = [(str(full_teams[rpi_r_s[0][sim] - 1]) + ", " + str(round(rpi_values_ns[rpi_r_s[0][sim] - 1], 5))) for sim in range(K)]
#for i in range(K):
    #print(str(i + 1) + ": " + rpi_final_list[i])
